...
down_current_project() {
  ...
  "${COMPOSE_CMD[@]}" down "${extra_args[@]}" >/dev/null 2>&1 || true
}
...
if [[ "${ACTION}" == "up" ]]; then
  down_all_family_projects
  echo "Deploying profile-service local stack (mode: ${DEPLOY_MODE}, project: ${COMPOSE_PROJECT_NAME})..."
  "${COMPOSE_CMD[@]}" up -d --build
  echo "profile-service URL: http://localhost:8080"
elif [[ "${ACTION}" == "down" ]]; then
  echo "Stopping profile-service local stack (mode: ${DEPLOY_MODE}, project: ${COMPOSE_PROJECT_NAME})..."
  down_all_family_projects
else
