...
if [[ "${ACTION}" == "up" ]]; then
  down_all_family_projects
  echo "Deploying cv-parser-service local stack (mode: ${DEPLOY_MODE}, project: ${COMPOSE_PROJECT_NAME})..."
  "${COMPOSE_CMD[@]}" up -d --build
  echo "cv-parser-service URL: http://localhost:8090"
elif [[ "${ACTION}" == "down" ]]; then
  echo "Stopping cv-parser-service local stack (mode: ${DEPLOY_MODE}, project: ${COMPOSE_PROJECT_NAME})..."
  down_all_family_projects
else
